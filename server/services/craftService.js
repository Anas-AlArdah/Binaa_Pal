...
const CRAFT_METADATA = [
  ...
  },
  {
    ...
    iconKey: 'painting',
    name: 'الدهان',
    description: 'دهان داخلي وخارجي وتشطيبات',
  ...
  },
  {
    ...
    iconKey: 'electrical',
    name: 'الكهرباء',
    description: 'تمديدات كهربائية وإنارة وصيانة',
  ...
  },
  {
    ...
    iconKey: 'plumbing',
    name: 'السباكة',
    description: 'تمديدات مياه وصرف صحي وسخانات',
  ...
  },
  {
    ...
    iconKey: 'gypsum',
    name: 'الجبس والأسقف',
    description: 'أسقف مستعارة وجبس بورد وديكورات',
  ...
  },
  {
    ...
    iconKey: 'carpentry',
    name: 'النجارة',
    description: 'أثاث مخصص وأبواب ومطابخ وأعمال خشبية',
  ...
  },
  {
    ...
    description: 'شبابيك وأبواب وأعمال معدنية',
  ...
  },
  {
    ...
    iconKey: 'masonry',
    name: 'البناء والحجر',
    description: 'أعمال حجر وبناء وجدران إنشائية',
...
function getWorkerLinkIdentity(workerIds, profileByUserId, profileById) {
  const profileIdMatches = workerIds.filter((workerId) => profileById.has(Number(workerId))).length;
  const userIdMatches = workerIds.filter((workerId) => profileByUserId.has(Number(workerId))).length;

  return profileIdMatches > userIdMatches ? 'profile_id' : 'user_id';
}

function resolveProfileForSkill(workerId, profileByUserId, profileById, linkIdentity = 'user_id') {
  const normalizedWorkerId = Number(workerId);
  const profileByUser = profileByUserId.get(normalizedWorkerId) || null;
  const profileByIdMatch = profileById.get(normalizedWorkerId) || null;

  if (linkIdentity === 'profile_id' && profileByIdMatch) {
    return profileByIdMatch;
  }
...
async function getWorkerCountBySkillId() {
  ...
  const linkIdentity = getWorkerLinkIdentity(workerIds, profileByUserId, profileById);
  ...
  skillLinks.forEach((linkModel) => {
    ...
    const profile = resolveProfileForSkill(workerId, profileByUserId, profileById, linkIdentity);
...
async function getCraftWorkers(slugOrId) {
  ...
  allSkillLinks.forEach((linkModel) => {
    const link = toPlain(linkModel);
    const skillName = link.skill?.skill_name;
    const profile = resolveProfileForSkill(link.worker_id, profileByUserId, profileById, linkIdentity);
